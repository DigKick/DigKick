...
test('save table to database', async () => {
  ...
  const searchedTable = await TableEntity.findOneBy({
    name: createValidTable().name,
  });
...
test('save multiple tables to database', async () => {
  ...
  const firstTableFromDb = await TableEntity.findOneBy({
    name: createValidTable().name,
  });
  const secondTableFromDb = await TableEntity.findOneBy({
