...
test('get player by serial number', async () => {
  ...
  const playerBySerialNum = await PlayerRepository.findPlayerByUnhashedKey(createValidPlayer().key)
...
test('hash validator', async () => {
  ...
  const isMatch = await SerialNumberHasher.hashValidator(hashedStr, serialNumber)
