...
test('parse valid payload to by id', () => {
  expect(() => RequestPayloadParser.parseRequestPayload(unparsedByIdPayload)).not.toThrow()
  const parsedPayload = RequestPayloadParser.parseRequestPayload(unparsedByIdPayload);
  expect(parsedPayload.type).toEqual(RequestPayloadType.BY_ID);
})

test('parse invalid payload to by id', () => {
  expect(() => RequestPayloadParser.parseRequestPayload(unparsedByIdInvalidPayload)).toThrow()
})

test('parse valid payload to by time span', () => {
  expect(() => RequestPayloadParser.parseRequestPayload(unparsedByTimeSpanPayload)).not.toThrow()
  const parsedPayload = RequestPayloadParser.parseRequestPayload(unparsedByTimeSpanPayload);
  expect(parsedPayload.type).toEqual(RequestPayloadType.BY_TIME);
})

test('parse invalid payload to by time span', () => {
  expect(() => RequestPayloadParser.parseRequestPayload(unparsedByTimeSpanInvalidPayload)).toThrow()
})

test('parse valid payload to by recent', () => {
  expect(() => RequestPayloadParser.parseRequestPayload(unparsedByRecentPayload)).not.toThrow()
  const parsedPayload = RequestPayloadParser.parseRequestPayload(unparsedByRecentPayload);
  expect(parsedPayload.type).toEqual(RequestPayloadType.BY_RECENT);
})

test('parse invalid payload by recent', () => {
  expect(() => RequestPayloadParser.parseRequestPayload(unparsedByRecentInvalidPayload)).toThrow()
})
