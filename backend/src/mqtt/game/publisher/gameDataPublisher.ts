...
export class GameDataPublisher extends DataPublisher {
  ...
  public async publishRecent() {
    const gamePromises = GameEntity.find({
      take: GameDataPublisher.recentItemCount,
    });
    ...
    gamePromises.then((games) => {
      ...
      DkMqttClient.getInstance().publishWithRetain(
        GameDataPublishTopics.RECENT,
        Obj2StringParser.objectToString(games) || '[]',
      );
    });
  }
  ...
  public async publishById(id: number) {
    ...
    if (!gameEntity) {
      DkMqttClient.getInstance().publish(
        GameDataPublishTopics.BASE + '/' + id,
        '{}',
      );
      return;
    }
