...
export class DataRequestHandler {
  ...
  public registerDataPublisher(type: BasicTerm, publisher: DataPublisher) {
    if (DataRequestHandler.dataPublisher.get(type)) {
      DataRequestHandler._logger.warn(
        `DataPublisher for '${type}' already registered.`,
      );
      return;
    }
  ...
  private static handleRequest(topic: string, payload: RequestPayload) {
    ...
    try {
      ...
      this._logger.warn(
        `Could not parse payload: ${JSON.stringify(payload)}: ${error}`,
      );
      return;
    }

    const topicTool = new TopicTool(topic);
    let matchingPublisher: DataPublisher | undefined = this.dataPublisher.get(
      topicTool.getSegment(1) as BasicTerm,
    );

    if (!matchingPublisher) {
      this._logger.warn(
        `No publisher for type ${topicTool.getSegment(1)} does not exist.`,
      );
      return;
    }
  ...
  private static _publishRequestPayload(
    parsedPayload: any,
    matchingPublisher: DataPublisher,
  ) {
    switch (parsedPayload.type.toString().toLowerCase()) {
      ...
      case RequestPayloadType.BY_TIME.toLowerCase():
        ...
        matchingPublisher
          .publishByTimeSpan(parsedPayload.from, parsedPayload.to)
          .then();
        break;
