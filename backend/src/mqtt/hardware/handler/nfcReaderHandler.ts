...
export class NfcReaderHandler extends DkModelHandler<
  NfcReaderEventType,
  TableHandler
> {
  ...
  private _topicToEventRouter = (topic: string, payload: any) => {
    ...
    try {
      ...
      this._logger.error(
        'Error while converting payload to NfcReaderPayload: ',
        e,
      );
      return;
    }
    ...
    this.triggerEvent(
      NfcReaderEventType.READER_NFC_TAG_SERIAL_NUMBER,
      topic,
      nfcReaderPayload,
    );
    return;
  ...
  constructor(subject: TableHandler, teamColor: TeamColor) {
    ...
    const nfcReaderTopicManager = new NfcReaderTopicManager(
      subject.subject,
      teamColor,
    );
