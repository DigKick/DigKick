...
export class SensorEventMapper implements EventMapper<SensorEventType> {
  ...
  map(event: SensorEventType, topic: string, payload: any) {
    ...
    switch (event) {
      case SensorEventType.BUTTON_0_LOW:
        this._tableHandler.triggerEvent(
          TableEventType.FINISH_GAME,
          topic,
          payload,
        );
    ...
    dkMqttClient.publishWithRetain(
      BaseTopicFactory.getBaseTopic(this._tableHandler.subject) + '/debug',
  ...
  private _updateLeds(teamColor: TeamColor) {
    ...
    for (
      let i = 0;
      i < this._tableHandler.subject.game.getTeamByColor(teamColor).score;
      i++
    ) {
      if (i % 2 == 0) {
    ...
    dkMqttClient.publish(
      BaseTopicFactory.getLedUpdateTopic(this._tableHandler.subject, teamColor),
      JSON.stringify(new LedUpdatePayload(ledColors, '').toJSON()),
    );
  }
